...
## v5.49
- CHINCHIROに役成立条件リファレンスを追加。
- QUICK/CUSTOM対戦ヘッダーから PLAYER A / RIVAL 表記を廃止し、装備中の称号を表示。
- VSカットインにも装備中称号を表示。
- JANKENは初回VSカットイン終了後に15秒タイマーUIが現れるよう変更。
- QUICK BATTLE 3種をスマホでも横3列表示。
- 0pt時の自動退場をホーム復帰後の段階的なアカウント消去演出に変更。サーバー側は60秒のフォールバック削除。
- 称号システムを追加。ROOKIE、ランキング1〜3位到達、各QUICK BATTLE 5/10/15戦を実装。獲得称号はアカウント削除まで保持、HOMEから1つ装備可能。
- HOMEのアバターをタップして12種から変更可能。
- 管理者ログを運営調整 / 対戦結果に分離し、新規ログは最大20件だけ保持。
...
## v5.54
- HOMEのPLAYER STATUSを「称号 → PLAYER ID → PLAYER NAME」の順に変更。
- 称号画像の外側黒背景を透過化し、HOME/プロフィール/称号一覧で背景になじむよう調整。
- 称号画像がモバイルで見切れないよう最大幅・高さ・object-fitを再調整。
- NETWORKプロフィールも「称号 → ID → 名前」の順に統一。
...
## v5.58
- HOMEの対戦導線を階層化：対戦マッチング → QUICK/CUSTOM → QUICKゲーム3種 → 対戦相手ID/対戦ポイント → 対戦申請。
- CUSTOMは対戦相手ID/審判ID/対戦ポイントを入力して申請する専用画面に変更。
- 生成済みのNEXUS専用UI画像へ差し替え、WebP軽量化して public/ui に配置。
- NETWORKは入口 → GLOBAL CHAT / FRIENDS / LIVE BATTLE の独立画面型UIに変更し、相互移動可能。
