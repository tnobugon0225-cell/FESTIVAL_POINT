...
function publicRandomMatch(r,viewer){if(!r)return null;const mine=Number(r.player1_id)===Number(viewer),ready=r.status==='ready';return {id:Number(r.id),status:r.status,wager:Number(r.wager),myChoice:mine?r.player1_choice:r.player2_choice,rivalLocked:mine?!!r.player2_choice:!!r.player1_choice,rivalChoice:ready?(mine?r.player2_choice:r.player1_choice):null,selectedGame:r.selected_game||null,quickMatchId:r.quick_match_id?Number(r.quick_match_id):null,selectionStartedAt:r.selection_started_at,path:r.selected_game?randomGamePath(r.selected_game):null}}
// v6.55: status is passive for old sessions. Never resurrect an abandoned RANDOM MATCH.
app.get('/api/random-match/status',requireUser,async(req,res,next)=>{const client=await pool.connect();try{await client.query('BEGIN');
  await client.query(`DELETE FROM random_match_queue WHERE joined_at < NOW()-INTERVAL '20 seconds'`);
  await client.query('UPDATE random_match_queue SET joined_at=NOW() WHERE user_id=$1',[req.userId]);
  // A selection screen is short-lived. Anything older than 45s is abandoned instead of being auto-finalized later.
  const stale=(await client.query(`SELECT id,quick_match_id FROM random_matches WHERE (player1_id=$1 OR player2_id=$1) AND status=ANY($2::varchar[]) AND created_at < NOW()-INTERVAL '45 seconds' FOR UPDATE`,[req.userId,['selecting','ready']])).rows;
  for(const x of stale){if(x.quick_match_id)await client.query(`UPDATE quick_matches SET status='cancelled' WHERE id=$1 AND status IN ('pending','setup','in_progress')`,[x.quick_match_id]);}
  await client.query(`UPDATE random_matches SET status='cancelled' WHERE (player1_id=$1 OR player2_id=$1) AND status=ANY($2::varchar[]) AND created_at < NOW()-INTERVAL '45 seconds'`,[req.userId,['selecting','ready']]);
  let rm=(await client.query(`SELECT * FROM random_matches WHERE (player1_id=$1 OR player2_id=$1) AND status=ANY($2::varchar[]) AND created_at >= NOW()-INTERVAL '45 seconds' ORDER BY id DESC LIMIT 1 FOR UPDATE`,[req.userId,['selecting','ready']])).rows[0];
  if(rm&&rm.status==='selecting')rm=await finalizeRandomMatch(client,rm);
  const c=(await client.query('SELECT COUNT(*)::int AS n FROM random_match_queue')).rows[0].n;const queued=!!(await client.query('SELECT 1 FROM random_match_queue WHERE user_id=$1',[req.userId])).rows[0];await client.query('COMMIT');res.json({waiting:Number(c),queued,match:publicRandomMatch(rm,req.userId)});}catch(e){try{await client.query('ROLLBACK')}catch{}next(e)}finally{client.release()}});
app.post('/api/random-match/join',requireUser,async(req,res,next)=>{const client=await pool.connect();try{await client.query('BEGIN');const me=(await client.query('SELECT id,points FROM users WHERE id=$1 FOR UPDATE',[req.userId])).rows[0];if(!me||Number(me.points)<10){await client.query('ROLLBACK');return res.status(400).json({error:'RANDOM MATCHには10pt以上必要です'})}if(await activeMatchForUser(req.userId,client)){await client.query('ROLLBACK');return res.status(409).json({error:'進行中の対戦があります'})}await client.query('DELETE FROM random_match_queue WHERE user_id=$1',[req.userId]);const rival=(await client.query(`SELECT q.user_id,u.points FROM random_match_queue q JOIN users u ON u.id=q.user_id WHERE q.user_id<>$1 AND u.points>=10 ORDER BY q.joined_at FOR UPDATE SKIP LOCKED LIMIT 1`,[req.userId])).rows[0];if(!rival){await client.query('INSERT INTO random_match_queue(user_id) VALUES($1) ON CONFLICT(user_id) DO UPDATE SET joined_at=NOW()',[req.userId]);await client.query('COMMIT');return res.json({ok:true,matched:false})}if(await activeMatchForUser(rival.user_id,client)){await client.query('DELETE FROM random_match_queue WHERE user_id=$1',[rival.user_id]);await client.query('INSERT INTO random_match_queue(user_id) VALUES($1) ON CONFLICT(user_id) DO NOTHING',[req.userId]);await client.query('COMMIT');return res.json({ok:true,matched:false})}await client.query('DELETE FROM random_match_queue WHERE user_id=$1',[rival.user_id]);const cap=Math.min(Number(me.points),Number(rival.points));const opts=[10,20,30].filter(x=>x<=cap);const wager=opts[Math.floor(Math.random()*opts.length)];const rm=(await client.query(`INSERT INTO random_matches(player1_id,player2_id,wager,selection_started_at) VALUES($1,$2,$3,NOW()+INTERVAL '3 seconds') RETURNING *`,[rival.user_id,req.userId,wager])).rows[0];await client.query('COMMIT');res.json({ok:true,matched:true,match:publicRandomMatch(rm,req.userId)});}catch(e){try{await client.query('ROLLBACK')}catch{}next(e)}finally{client.release()}});
...
// v6.55: leaving a RANDOM MATCH for HOME invalidates it immediately.
app.post('/api/random-match/leave',requireUser,async(req,res,next)=>{const client=await pool.connect();try{await client.query('BEGIN');
  await client.query('DELETE FROM random_match_queue WHERE user_id=$1',[req.userId]);
  const rows=(await client.query(`SELECT id,quick_match_id FROM random_matches WHERE (player1_id=$1 OR player2_id=$1) AND status IN ('selecting','ready') FOR UPDATE`,[req.userId])).rows;
  for(const x of rows){if(x.quick_match_id)await client.query(`UPDATE quick_matches SET status='cancelled' WHERE id=$1 AND status IN ('pending','setup','in_progress')`,[x.quick_match_id]);}
  await client.query(`UPDATE random_matches SET status='cancelled' WHERE (player1_id=$1 OR player2_id=$1) AND status IN ('selecting','ready')`,[req.userId]);
...
function evalChinchiro(dice){
  ...
  if(s[0]===1&&s[1]===2&&s[2]===3)return {role:'hifumi',value:1,label:'HIFUMI / 1-2-3'};
