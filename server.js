...
app.post('/api/random-match/leave',requireUser,async(req,res,next)=>{const client=await pool.connect();try{await client.query('BEGIN');await client.query('DELETE FROM random_match_queue WHERE user_id=$1',[req.userId]);const rm=(await client.query(`SELECT * FROM random_matches WHERE (player1_id=$1 OR player2_id=$1) AND status=ANY($2::varchar[]) ORDER BY id DESC LIMIT 1 FOR UPDATE`,[req.userId,['selecting','ready']])).rows[0];if(rm){if(rm.quick_match_id){await client.query(`UPDATE quick_matches SET status='abandoned' WHERE id=$1 AND status=ANY($2::varchar[])`,[rm.quick_match_id,['pending','setup','in_progress']]);}await client.query(`UPDATE random_matches SET status='abandoned' WHERE id=$1`,[rm.id]);}await client.query('COMMIT');res.json({ok:true});}catch(e){try{await client.query('ROLLBACK')}catch{}next(e)}finally{client.release()}});
