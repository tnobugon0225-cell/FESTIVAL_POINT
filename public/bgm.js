(()=>{
  ...
  function ensureSettings(){
    ...
    settingsPanel=document.createElement('div');settingsPanel.className='nexus-settings-panel';settingsPanel.innerHTML=`<div class="nexus-settings-card"><div class="nexus-settings-head"><div><span>SOUND CONTROL</span><strong>SETTING</strong></div><button type="button" data-no-se class="nexus-settings-close asset-close" aria-label="閉じる"><img src="/ui/v576/close.webp" alt="閉じる"></button></div>${sliderRow('master','MASTER')}${sliderRow('bgm','BGM')}${sliderRow('se','SE')}<p>端末の音量とは別に、NEXUS:ZERO内の音量を調整します。</p></div>`;
